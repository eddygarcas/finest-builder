...
class Finest::StructTest < Minitest::Test
  ...
  def test_struct_getting_as_json
    el = MyStruct.new(json: @simple_struct)
  ...
  def test_using_builder_sym_keys
    elements = MyStruct.new.build_by_keys(json: {"id" => 123, text: "gathering"}, keys: ["id", :text])
    obj_builder = MyStruct.new(json: elements.to_h)
  ...
  def test_builder_directly
    element = MyStruct.new(json: {"id" => 123, text: "gathering"})
